...
if ($exec && $row > 0) {
	$sql = 'SELECT * FROM avaliacao WHERE id_filme_fk = ? AND id_user_fk = ? AND desliked = 1';
	$stmt = $conn->prepare($sql);
	$stmt->bindParam(1, $id_filme);
	$stmt->bindParam(2, $id_user);
	$exec = $stmt->execute();
	...
	if($exec && $row > 0){
		...
		$sql = 'UPDATE avaliacao SET liked = 0, desliked = 1 WHERE id_filme_fk = ? AND id_user_fk = ?';
		$stmt = $conn->prepare($sql);
		$stmt->bindParam(1, $id_filme);
		$stmt->bindParam(2, $id_user);
		$exec = $stmt->execute();
	...
	$sql = 'INSERT INTO avaliacao (id_filme_fk, id_user_fk, desliked) VALUES (?,?,?)';
	$stmt = $conn->prepare($sql);
	$stmt->bindParam(1, $id_filme);
	$stmt->bindParam(2, $id_user);
	$stmt->bindParam(3, $deslike);
	$stmt->execute();
